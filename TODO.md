...
- [ ] **P4-3**（可选）实时演示模式（未做；离线版已覆盖"过程可视化"主目标）
- [x] **P4-4** 伦理约束落地：CLI `--consent` 强制确认门（缺省拒绝运行）+ `demos/faceswap/README.md` 三条使用边界声明（0.25d）

**Phase 4 验收**：几何纯函数单测 6 个全绿（剖分计数/边界、平移恢复、Reinhard 均值迁移、端到端区域改变、线框绘制）；真图自换脸三联对照统计一致（161.8/162.0/161.5，源=目标时结果≈自身，管线自洽）；双人脸素材实拍由用户线下验证。
